...
def load_training_frame(path: str | Path = "data/sample/events.csv") -> Tuple[pd.DataFrame, pd.Series]:
...
def split_data(features: pd.DataFrame, target: pd.Series, test_size: float = 0.2, seed: int = 42):
    return train_test_split(features, target, test_size=test_size, random_state=seed, stratify=target)
